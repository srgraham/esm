...
func RegisterExt(id int8, value interface{}) {
	...
	if _, ok := extTypes[id]; ok {
		panic(fmt.Errorf("esm: ext with id=%d is already registered", id))
	}
	extTypes[id] = typ
...
func (d *Decoder) parseExtLen(c byte) (int, error) {
	...
	case codes.Ext32:
		...
		return int(n), err
	default:
		return 0, fmt.Errorf("esm: invalid code=%x decoding ext length", c)
	}
}
...
func (d *Decoder) ext(c byte) (interface{}, error) {
	...
	if !ok {
		return nil, fmt.Errorf("esm: unregistered ext id=%d", extId)
	}
