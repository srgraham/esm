...
func (d *Decoder) decode(dst interface{}) error {
	...
	if !v.IsValid() {
		return errors.New("esm: Decode(nil)")
	}
	if v.Kind() != reflect.Ptr {
		return fmt.Errorf("esm: Decode(nonsettable %T)", dst)
	}
	v = v.Elem()
	if !v.IsValid() {
		return fmt.Errorf("esm: Decode(nonsettable %T)", dst)
...
func (d *Decoder) DecodeNil() error {
	...
	if c != codes.Nil {
		return fmt.Errorf("esm: invalid code=%x decoding nil", c)
	}
	return nil
}
...
func (d *Decoder) bool(c byte) (bool, error) {
	...
	return false, fmt.Errorf("esm: invalid code=%x decoding bool", c)
}
...
func (d *Decoder) DecodeInterface() (interface{}, error) {
	...
	return 0, fmt.Errorf("esm: unknown code %x decoding interface{}", c)
}
...
func (d *Decoder) Skip() error {
	...
	return fmt.Errorf("esm: unknown code %x", c)
}

// peekCode returns next MessagePack code. See
// https://github.com/esm/esm/blob/master/spec.md#formats for details.
...
func (d *Decoder) Peek(params ...int) (bytes []byte, err error) {
	byte_length := 80
	is_eof := false
	if len(params) == 1 {
		byte_length = params[0]
	}
	if d.r.Buffered() > byte_length {
		is_eof = true
		byte_length = d.r.Buffered()
	}
	bytes, err = d.r.Peek(byte_length)

	re := regexp.MustCompile("[^a-z0-9_!@#$^&*()+= -]")

	s := string(re.ReplaceAll(bytes, []byte("."))[:])
