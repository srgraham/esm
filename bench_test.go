...
var _ esm.CustomEncoder = (*benchmarkStruct2)(nil)
var _ esm.CustomDecoder = (*benchmarkStruct2)(nil)

func (s *benchmarkStruct2) EncodeEsm(enc *esm.Encoder) error {
	return enc.Encode(
		s.Name,
		s.Colors,
		s.Age,
...
func (s *benchmarkStruct2) DecodeEsm(dec *esm.Decoder) error {
	return dec.Decode(
		&s.Name,
		&s.Colors,
		&s.Age,
...
func BenchmarkStructVmihailencoEsm(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		buf, err := esm.Marshal(in)
		if err != nil {
			b.Fatal(err)
		}

		err = esm.Unmarshal(buf, out)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkCSVEsm(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var num int
...
func BenchmarkQuery(b *testing.B) {
	...
	bs, err := esm.Marshal(records)
	if err != nil {
		b.Fatal(err)
	}

	dec := esm.NewDecoder(bytes.NewBuffer(bs))
